            ...
            >
              {React.Children.map(children, (child, index) => {
                if (React.isValidElement(child) && child.type === GlowingCard) {
                  const typedChildProps = child.props as GlowingCardProps & { style?: React.CSSProperties; className?: string };
                  const cardGlowColor = typedChildProps.glowColor || "#3b82f6";
                  return React.cloneElement(child as React.ReactElement<any>, {
                    className: cn(
                      typedChildProps.className,
                      "bg-opacity-15 dark:bg-opacity-15",
                      "border-opacity-100 dark:border-opacity-100"
                    ),
                    style: {
                      ...typedChildProps.style,
