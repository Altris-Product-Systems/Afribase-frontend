...
export default function TableEditor({ projectId }: TableEditorProps) {
    ...
    const ColumnBadge = ({ col }: { col: any }) => (
        <div className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-900/50 border-b border-zinc-800/50 transition-colors">
            <div className="flex items-center gap-1.5 w-5 shrink-0">
                {col.primaryKey && <Key size={12} className="text-amber-500" />}
                {col.foreignKey && <Link size={12} className="text-blue-500" />}
                {!col.primaryKey && !col.foreignKey && <span className="w-3 h-3 rounded-full border border-zinc-700" />}
            </div>
            <span className="font-medium text-sm text-zinc-100 w-40 truncate">{col.name}</span>
            <code className="text-xs bg-zinc-800 px-2 py-0.5 rounded text-zinc-400 font-mono">{col.type}</code>
    ...
    return (
        <div className="flex h-full gap-4" style={{ minHeight: '75vh' }}>
            {/* Table list sidebar */}
            <div className="w-56 shrink-0 border border-zinc-800 rounded-xl bg-zinc-950 overflow-hidden flex flex-col">
                <div className="p-3 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
                    ...
                    <button onClick={load} disabled={loading} className="p-1 hover:bg-zinc-700 rounded transition-colors">
                    ...
                    {loading ? (
                        ...
                        tables.map(t => (
                            <button key={t.name} onClick={() => setSelectedTable(t.name)}
                                className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${selectedTable === t.name ? 'bg-emerald-500/10 text-emerald-400 font-semibold' : 'text-zinc-400 hover:bg-zinc-800'}`}>
                                <Table size={13} />
                                    ...
                                    <div className="space-y-2">
                                        {activeTable.indexes.map((idx: any) => (
                                            <div key={idx.name} className="flex items-center gap-3 text-xs">
                                                ...
                                                <span className="font-medium text-zinc-300">{idx.name}</span>
