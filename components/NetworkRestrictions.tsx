...
export default function NetworkRestrictions({ projectId }: NetworkRestrictionsProps) {
    ...
    return (
        <div className="space-y-6 max-w-2xl">
            ...
            <div className="border border-zinc-200 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-950 p-6 space-y-6">
                ...
                {enabled && (
                    <>
                        <div>
                            <h4 className="font-semibold text-sm mb-3 text-emerald-600 dark:text-emerald-400">✅ Allow List (CIDR ranges)</h4>
                            <div className="space-y-2 mb-3">
                                {allowList.map((ip, i) => (
                                    <div key={i} className="flex items-center gap-2">
                                        <code className="flex-1 text-xs bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-3 py-2 rounded font-mono">{ip}</code>
                            ...
                            <div className="flex gap-2">
                                <input value={newAllow} onChange={e => setNewAllow(e.target.value)} placeholder="192.168.1.0/24 or 0.0.0.0/0"
                                    className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-2 text-sm font-mono rounded focus:outline-none focus:border-emerald-500" />
                        ...
                        <div>
                            <h4 className="font-semibold text-sm mb-3 text-red-600 dark:text-red-400">🚫 Deny List</h4>
                            <div className="space-y-2 mb-3">
                                {denyList.map((ip, i) => (
                                    <div key={i} className="flex items-center gap-2">
                                        <code className="flex-1 text-xs bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-3 py-2 rounded font-mono">{ip}</code>
                            ...
                            <div className="flex gap-2">
                                <input value={newDeny} onChange={e => setNewDeny(e.target.value)} placeholder="10.0.0.5/32"
                                    className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-2 text-sm font-mono rounded focus:outline-none focus:border-red-500" />
