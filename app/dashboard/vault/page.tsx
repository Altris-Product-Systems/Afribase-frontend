...
import { Lock, ChevronRight, Database } from 'lucide-react';
...
export default function VaultPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Unlocking Secret Vaults');
    setIsLoading(true);
    try {
      ...
      // console.error('Failed to load projects for vault', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=vault${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Vault secrets are managed per project. You'll need to create a project before storing encrypted secrets and credentials."
          icon={Lock}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to manage encrypted secrets, API keys, and sensitive credentials with column-level encryption.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Manage Secrets</span>
