...
export default function CronPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Syncing Cron Schedules');
    setIsLoading(true);
    try {
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=cron${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Cron jobs are managed per project. You'll need to create a project before scheduling recurring tasks."
          icon={Activity}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to create and manage scheduled cron jobs that run on a recurring basis.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Manage Crons</span>
