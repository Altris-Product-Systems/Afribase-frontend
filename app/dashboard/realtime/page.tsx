...
export default function RealtimePage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Initializing Realtime Channels');
    setIsLoading(true);
    try {
      ...
      console.error('Failed to load projects for realtime', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=realtime${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Realtime subscriptions are managed per project. You'll need to create a project before configuring WebSocket channels and broadcast settings."
          icon={Radio}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to configure real-time subscriptions, presence tracking, and broadcast channels.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Configure Realtime</span>
