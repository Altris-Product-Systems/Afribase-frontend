...
import { ChevronRight, FileCode, Database } from 'lucide-react';
...
export default function MigrationsPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Loading Migration History');
    setIsLoading(true);
    try {
      ...
      console.error('Failed to load projects for migrations', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=migrations${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Migrations are managed per project. You'll need to create a project before tracking schema changes."
          icon={FileCode}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to track and manage version-controlled database schema changes.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">View Migrations</span>
