...
import { Archive, ChevronRight, Database } from 'lucide-react';
...
export default function BackupsPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Scanning Backup Vaults');
    setIsLoading(true);
    try {
      ...
      console.error('Failed to load projects for backups', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=backups${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Backups are managed per project. You'll need to create a project before scheduling or restoring backups."
          icon={Archive}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to schedule automatic backups and restore your database to any point in time.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Manage Backups</span>
