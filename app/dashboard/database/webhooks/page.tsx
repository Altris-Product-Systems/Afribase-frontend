...
import { Webhook, ChevronRight, Database } from 'lucide-react';
...
export default function WebhooksPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Indexing Webhook Endpoints');
    setIsLoading(true);
    try {
      ...
      // console.error('Failed to load projects for webhooks', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=webhooks${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Webhooks are configured per project. You'll need to create a project before setting up database event triggers."
          icon={Webhook}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to configure HTTP webhooks that fire on database INSERT, UPDATE, and DELETE events.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Configure Hooks</span>
