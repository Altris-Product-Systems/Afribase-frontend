...
export default function UsagePage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Aggregating Usage Metrics');
    setIsLoading(true);
    try {
      ...
      // console.error('Failed to load projects for usage', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=usage${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Usage metrics are tracked per project. You'll need to create a project before monitoring resource consumption."
          icon={BarChart3}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to view detailed resource usage, bandwidth consumption, and billing metrics.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">View Usage</span>
