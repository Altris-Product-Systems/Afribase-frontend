...
import { Shield, ChevronRight, Database } from 'lucide-react';
...
export default function NetworkPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Scanning Network Policies');
    setIsLoading(true);
    try {
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=network${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Network restrictions are managed per project. You'll need to create a project before configuring IP allow-lists and firewall rules."
          icon={Shield}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to configure IP allow-lists, firewall rules, and network access policies.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Network Rules</span>
