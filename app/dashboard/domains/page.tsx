...
export default function DomainsPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Resolving Custom Domains');
    setIsLoading(true);
    try {
      ...
      // console.error('Failed to load projects for domains', err);
    } finally {
      setIsLoading(false);
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=domains${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="Custom domains are configured per project. You'll need to create a project before adding custom domain mappings."
          icon={Globe}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to configure custom domain names with automatic SSL certificate provisioning.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Manage Domains</span>
