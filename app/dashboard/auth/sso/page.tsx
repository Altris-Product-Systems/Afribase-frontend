...
export default function SSOPage() {
  ...
  const loadProjects = async () => {
    setGlobalLoading(true, 'Querying Identity Providers');
    setIsLoading(true);
    try {
  ...
  const handleSelectProject = (projectId: string) => {
    router.push(`/dashboard/project/${projectId}?tab=sso${orgId ? `&orgId=${orgId}` : ''}`);
  };
    ...
    return (
      <div className="p-8 lg:p-10">
        <EmptyState
          ...
          description="SSO / SAML configuration is managed per project. You'll need to create a project before connecting identity providers."
          icon={ShieldCheck}
    ...
    <div className="p-8 lg:p-10 max-w-7xl mx-auto space-y-12 animate-fade-in">
      <div className="space-y-4">
        ...
        <p className="text-zinc-400 text-sm max-w-xl leading-relaxed font-medium">
          Select a project node to configure Single Sign-On providers and SAML integrations for enterprise authentication.
        </p>
      </div>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Configure SSO</span>
